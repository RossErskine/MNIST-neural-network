...
namespace NN
{
	...
	*///=====================================================================
	void Network::trainNetwork(std::vector<double> dataSet, int label)
	{
		...
		{
			mOutput_layer[i].setInput(forward_prpogation(mHidden_layer, i));
			//std::cout << "Output: " << mOutput_layer[i].getInput() << '\n';
		}
	...
	*///====================================================================
	int Network::final_guess()
	{
		...
		for (int i = 0; i != mLocalError.size(); i++)
		{
			...
			if (mLocalError[i] > comp && mLocalError[i] <= 1)
			{
				comp = mLocalError[i];
				guess = i;
			}
	...
	*///====================================================================
	...
	{
		const std::string NN_data_file = "NN_data_file_005.csv";
